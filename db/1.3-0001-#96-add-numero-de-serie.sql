...
ADD COLUMN `numero_de_serie` VARCHAR(45) NULL DEFAULT NULL AFTER `duree_de_vie_moins_18`;
...
VIEW `product_view` AS
    SELECT 
        ...
        `product`.`numero_de_serie` AS `numero_de_serie`,
...
CREATE DEFINER=`workshape`@`%` PROCEDURE `product_add`(in qr_code_ nvarchar(45),
    ...
    in numero_de_serie_ nvarchar(45),
	in event_label_ nvarchar(45),
	in name_label_ nvarchar(45))
BEGIN

    insert into product(qr_code, name, date_creation, fournisseur, ref_fournisseur, longueur_initiale, longueur_actuelle, largeur, grammage, type_de_tissus, date_arrivee, transport_frigo, lieu_actuel, lieu_depuis, temps_hors_gel_total, finished, duree_de_vie_20, duree_de_vie_moins_18, numero_de_serie)
		values(qr_code_,
            ...
            inner_nullify_str(numero_de_serie_));
...
BEGIN
    ...
    update product
		set name = inner_nullify_str(name_),
            ...
            numero_de_serie = inner_nullify_str(numero_de_serie_)
